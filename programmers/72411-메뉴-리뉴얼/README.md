...
1. 손님마다 주문한 목록을 오름차순으로 정렬합니다. 이는 아래 조건을 만족시키는 조합을 생성하기 위한 전처리입니다.
   
   > 정답은 각 코스요리 메뉴의 구성을 문자열 형식으로 배열에 담아 사전 순으로 ```오름차순``` 정렬해서 return 해주세요.

2. 가장 많은 요리를 주문한 손님이 주문한 단품요리의 개수를 구합니다.
3. 주어진 코스요리를 구성하는 단품메뉴들의 개수 중 최대 단품요리 개수보다 많은 경우 해당 개수의 코스요리는 만들 수 없습니다. 만들 수 없는 코스요리를 제거합니다.
4. 단품요리를 조합하여 주어진 개수의 단품요리로 구성된 코스요리를 만듭니다.
5. 가장 많이 주문될 코스요리가 몇 번 주문될 지 구합니다. 이는 아래 조건을 만족시키는 조합을 메뉴에 추가하기 위한 전처리입니다.
   
   > 또한, 최소 2명 이상의 손님으로부터 주문된 단품메뉴 조합에 대해서만 코스요리 메뉴 후보에 포함하기로 했습니다.

6. 가장 많이 주문될 코스요리의 예상 주문횟수가 2 이상인 경우, 해당 예상 주문횟수를 가진 코스요리를 메뉴에 추가합니다.
