...
```
### 수행 과정
0. 세로로 쌓여있는 각 줄을 우리는 스택(stack)으로 생각해볼 수 있습니다. 인형을 뽑는 행위를 각 열에서 가장 위에 있는 인형을 제거하는 방식으로 구현할 수 있습니다.
1. 행 중심으로 표현되어있는 `board`를 열 중심으로 바꿔줍니다. 또한, `pop()` 또는 `del`을 통해 가장 위에 있는 원소를 쉽게 빼줄 수 있도록, 가장 위에 있는 인형이 list의 가장 마지막 원소가 되도록 reverse해줍니다.
2. 인형을 뽑을 때,
   1. 해당 열에 뽑을 수 있는 인형이 더이상 없다면 인형을 뽑지 않습니다.
   2. 해당 열에 인형이 있다면 뽑고, 뽑은 인형은 바구니(`stack`)에 쌓아줍니다.
   3. 만약 바구니에 이전에 뽑은 인형이 적어도 한 개 쌓여있고, 이번에 뽑아 옮긴 인형이 바구니의 가장 위쪽 인형과 동일한 인형이라면 두 인형을 제거해주고, 사라진 인형 수를 2 증가시켜줍니다.
