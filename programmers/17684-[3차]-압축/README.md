...
1. 문제에서 주어진 기본 사전을 생성합니다.
2. 두 포인터를 각각 주어진 메시지의 시작과 다음 글자를 가리키도록 정의합니다.
3. 모든 글자를 확인할 때까지 아래 작업을 수행합니다:
   1. 부분메시지는 주어진 메시지의 left부터 (right - 1)까지의 문자열입니다.
   2. 만약 부분메시지가 사전에 없을 경우, 사전에 해당 부분메시지를 추가하고 다음 부분메시지를 보기 위해 left 포인터를 현재 부분메시지의 끝으로 옮깁니다.
   3. 현재 부분메시지의 다음 글자를 확인하기 위해 right 포인터를 1 증가시킵니다.
